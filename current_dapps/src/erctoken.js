...
import ErcToken from './artifacts/contracts/ErcToken.sol/TICKETTOKEN.json';


// contract address
const erctokenaddress = `0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512`;
 ...
 // CHECK TOKEN BALANCE
  ...
  async function checktokenBalance(address){
    ...
    if (window.ethereum) {
        ...
        // The Contract object
        const ContractObject = new ethers.Contract(erctokenaddress,ErcToken.abi, provider);
  
        console.log(ContractObject)
  
        try {
  
          const data = await ContractObject.balanceOf(address);
  
          console.log(data, 'data');

          let ethBalance = ethers.utils.formatEther(data)
  ...
  return (
    ...
    <div>
      ...
      <input onChange={e =>checktokenBalance(e.target.value)} placeholder="address token balance" />
